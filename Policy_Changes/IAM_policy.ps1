...
$ConditionExpression = "resource.name.startsWith('projects/_/buckets/med-labs-42f13/objects/public/')"
$ConditionTitle = "Public Folder Access"
...
if ($LASTEXITCODE -ne 0) {
    ...
    if ($papStatus -eq "enforced") {
        Write-Host "PAP is enforced, likely due to an organization policy. Cannot proceed with allUsers binding."
        exit 1
    }
} else {
...
# Add a delay to allow changes to propagate (increased to 30 seconds)
...
# Execute the gcloud command to add the IAM policy binding
Write-Host "Adding IAM policy binding for public access to public/ folder..."
gcloud storage buckets add-iam-policy-binding $BucketName `
  --member=$Member `
  --role=$Role `
  --condition="expression=$ConditionExpression,title='$ConditionTitle'"
